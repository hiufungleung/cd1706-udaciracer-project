		...
		*/
		try {
			...
			const raceInterval = setInterval(() => {
				getRace(raceID).then(data => {res = data})
				if (res != undefined) {
...
function getTracks() {
	...
	return fetch(`${SERVER}/api/tracks`, defaultFetchOpts()).then(
		response => {
			if (!response.ok) {
				throw new Error(`Get tracks error with status code ${response.status}`);
			}
...
function getRacers() {
	...
	return fetch(`${SERVER}/api/cars`, defaultFetchOpts()).then(
		response => {
			if (!response.ok) {
				throw new Error(`Get cars error with status code ${response.status}`);
			}
			return response.json();
		}
	).catch(
...
function getRace(id) {
	...
	return fetch(`${SERVER}/api/races/${id}`, defaultFetchOpts()).then(
		response => {
			if (!response.ok) {
				throw new Error(`Get cars error with status code ${response.status}`);
			}
